...
class Container implements ArrayAccess, ContainerContract
{
     ...
     */
    /*
     * Bind 方法不仅可以创建alias，还可以封装闭包函数，并把所有的对应绑定都存入到 bindings 数组中，如果一旦某个对象解析过了依然调用 bind 方法，那么就会 rebound 反弹，通过监听者的回调函数更新对象
     */
    public function bind($abstract, $concrete = null, $shared = false)
    {
        /*
         * 如果$abstract为数组
         * $abstract = ['event'=>'ev']; 抽象,别名
         * dd($this->aliases);
         * $this->aliases['env'] = 'event'
         */
        ...
        /*
         * $abstract为非数组时,删除$this->instance[$abstract]和$this->aliases[$abstract]
         * 目的是将$concrete包装为闭包.
         */
        ...
        /*
         * 若concrete不是闭包函数,则根据值是否等于$abstract 判断用build(=)还是make(<>)
         */
        if (! $concrete instanceof Closure) {
             ...
             * $concrete = function ($c, $parameters = []) use ($abstract, $concrete) {
                   return $c->build($concrete, $parameters); //塑造class的实例
               };
             *$concrete != $abstract
             *$concrete = function ($c, $parameters = []) use ($abstract, $concrete) {
                   return $c->make($concrete, $parameters);//make
               };
             *
             */
        }
        /*
         * $this->bindings结构类似于 ['event'=>['concrete'=>$closure,'shared'=>$bool],...]
         */
        ...
        //两种情况下$abstract被认为之前已解析,当前实例将会绑定到与之相关的listener
        //1,$abstract类已经被实例化过即$this->instances[$abstract]不为空
        //2,$this->rebounds[$abstract]不为空
    ...
    public function make($abstract, array $parameters = [])
    {
       ...
       //单例共享直接返回
        ...
        //检测是否可以进行实例化,isBuildable:($concrete == $abstract 或者 $concrete instanceof Closure)
        if ($this->isBuildable($concrete, $abstract)) {
            $object = $this->build($concrete, $parameters);
        } else {
            //用改变过后的$concrete再次获取buildings里面的instance
        ...
        $this->resolved[$abstract] = true; //resolved数组标记为已解析
    ...
    protected function getConcrete($abstract)
    {
        ...
        if (! isset($this->bindings[$abstract])) {
            if ($this->missingLeadingSlash($abstract) &&
                ...
                $abstract = '\\'.$abstract;
                //若abstract头没带'\',而bindings里面已经绑定了含有'\'的对象和实例则返回binding中的带斜线抽象.
            }
     ...
     */
    /*
     * 当$this->bindings[$abstract],不存在时,判断给定的$abstract头部是否是斜线
     */
    ...
    {
       /*
        * build以$this(即app())为参数,解析所有的闭包绑定.替换$app,$c之类的噱头.
        */
        if ($concrete instanceof Closure) {
        ...
        /*
         * 如果$concrete是接口或者抽象类或者其他未被bind过的字符串,则build抛出不可实例化的异常;
         */
        ...
        /*
         * 若没有构造函数，则从栈类弹出刚才压入的实体，实例化这个实体类并返回.
         */
        ...
        array_pop($this->buildStack);
        /*
         * 创建一个类的新实例，给出的参数将传递到类的构造函数,此处参数为app和route对象构成的数组.
         */
     ...
     */
    /*
     * 若$value是闭包,则直接绑定到bingdings否则将a包装为闭包函数再绑定.
     */
